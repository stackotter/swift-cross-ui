...
addSignal(name: "begin", handler: gCallback(handler0)) { [weak self] (param0: OpaquePointer) in
...
addSignal(name: "cancel", handler: gCallback(handler1)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "end", handler: gCallback(handler2)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
    self.end?(self, param0)
}

let handler3: @convention(c) (UnsafeMutableRawPointer, OpaquePointer, GtkEventSequenceState, UnsafeMutableRawPointer) -> Void =
    { _, value1, value2, data in
        SignalBox2<OpaquePointer, GtkEventSequenceState>.run(data, value1, value2)
    }

addSignal(name: "sequence-state-changed", handler: gCallback(handler3)) { [weak self] (param0: OpaquePointer, param1: GtkEventSequenceState) in
    guard let self = self else { return }
...
addSignal(name: "update", handler: gCallback(handler4)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::n-points", handler: gCallback(handler5)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::window", handler: gCallback(handler6)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
