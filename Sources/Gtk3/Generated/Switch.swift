...
let handler1: @convention(c) (UnsafeMutableRawPointer, Bool, UnsafeMutableRawPointer) -> Void =
    { _, value1, data in
...
addSignal(name: "notify::state", handler: gCallback(handler3)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::related-action", handler: gCallback(handler4)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::use-action-appearance", handler: gCallback(handler5)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
