...
public enum PolicyType {
    ...
    /// `GtkTreeView` can not be seen.
    case automatic
    /// The scrollbar should never appear. In this mode the content determines the size.
    case never
    /// Don’t show a scrollbar, but don’t force the size to follow the content. This can be used
    /// e.g. to make multiple scrolled windows share a scrollbar. Since: 3.16.
    case external
