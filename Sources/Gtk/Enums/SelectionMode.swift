...
///
/// [Gtk docs](https://docs.gtk.org/gtk3/enum.SelectionMode.html)
public enum SelectionMode {
    /// No selection is possible.
    case none
    /// Zero or one element may be selected.
    case single
    /// Exactly one element is selected. In some circumstances, such as initially or during a search
    /// operation, it’s possible for no element to be selected with GTK_SELECTION_BROWSE. What is
    /// really enforced is that the user can’t deselect a currently selected element except by
    /// selecting another element.
    case browse
    /// Any number of elements may be selected. The Ctrl key may be used to enlarge the selection,
    /// and Shift key to select between the focus and the child pointed to. Some widgets may also
