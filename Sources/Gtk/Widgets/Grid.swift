...
public class Grid: Widget {
    ...
    public func attach(child: Widget, left: Int, top: Int, width: Int, height: Int) {
        ...
        gtk_grid_attach(castedPointer(), child.widgetPointer, gint(left), gint(top), gint(width), gint(height))
    }

    public func attach(nextTo child: Widget, sibling: Widget, side: PositionType, width: Int, height: Int) {
        widgets.append(child)
        child.parentWidget = self

        gtk_grid_attach_next_to(castedPointer(), child.widgetPointer, sibling.widgetPointer, side.toGtkPositionType(), gint(width), gint(height))
    }
    ...
    /// The new row or column is placed next to `sibling`, on the side determined by `side`. If `side` is `.top` or `.bottom`, a row is inserted. If side is `.left` or `.right`, a column is inserted.
    /// - Parameter sibling: The child of `grid` that the new row or column will be placed next to.
    ...
    public func setRowBaselinePosition(forRow row: Int, position: BaselinePosition) {
        gtk_grid_set_row_baseline_position(castedPointer(), gint(row), position.toGtkBaselinePosition())
    }
}
