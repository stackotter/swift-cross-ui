...
open class NativeDialog: GObject {
    ...
    let handler0: @convention(c) (UnsafeMutableRawPointer, Int, UnsafeMutableRawPointer) -> Void =
    { _, value1, data in
...
addSignal(name: "notify::modal", handler: gCallback(handler1)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::title", handler: gCallback(handler2)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::transient-for", handler: gCallback(handler3)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::visible", handler: gCallback(handler4)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
