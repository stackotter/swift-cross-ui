...
public convenience init() {
    ...
    let handler0: @convention(c) (UnsafeMutableRawPointer, Int, Int, UnsafeMutableRawPointer) -> Void =
    { _, value1, value2, data in
        SignalBox2<Int, Int>.run(data, value1, value2)
    }

addSignal(name: "resize", handler: gCallback(handler0)) { [weak self] (param0: Int, param1: Int) in
...
addSignal(name: "notify::content-height", handler: gCallback(handler1)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
...
addSignal(name: "notify::content-width", handler: gCallback(handler2)) { [weak self] (param0: OpaquePointer) in
    guard let self = self else { return }
