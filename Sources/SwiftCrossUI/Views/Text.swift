...
/// use ``fixedSize`` on text, what they really need is a ``ScrollView``.
///
...
/// that SwiftCrossUI's layout system behaves rather unintuitively with views that
/// trade off width for height. The layout system used to support this behaviour
/// well, but when overhauling the layout system with performance in mind, we
/// discovered that it's not possible to handle minimum view sizing in the
/// intuitive way that we were, without a large performance cost or layout system
/// complexity cost.
///
...
/// that's what most UI frameworks do when text is given a small width). This leads
/// to the window thinking that its minimum height is `characterCount * lineHeight`,
/// even though when given a width larger than zero, the text view would be shorter
/// than this 'minimum height'. The underlying cause is the assumption that
/// 'minimum size' is a sensible notion for every view. A text view without
/// truncation doesn't have a 'minimum size'; are we minimizing width? minimizing
/// height? minimizing width + height? minimizing area?
///
...
/// such that the height of the text became taller than the window, then the window
/// would become taller, and if you resized the height of the window then you'd reach
/// the window's minimum height before the text could overflow the window horizontally.
/// Unfortunately this required a lot of book-keeping, and was deemed to be unfeasible
/// to do without significantly hurting performance due to all the layout assumptions
/// that we'd have to drop from our stack layout algorithm.
///
